...
* User must specify which mask
  ...
  if (type = 'L' | type = 'l') 
    if (lores) ; file = './cnlib/lpoly.asc' ; endif
    if (hires) ; file = './cnlib/lpoly_hires.asc' ; endif
  endif
  if (type = 'O' | type = 'o') 
    if (lores) ; file = './cnlib/opoly.asc' ; endif
    if (hires) ; file = './cnlib/opoly_hires.asc' ; endif
  endif
  if (type = 'cn' )    ; file = './cnlib/mask_cn.txt' ; endif
  if (type = 'cnout' | type = 'CNOUT' )
   file = './cnlib/cn_out' 
  endif
  if (type = 'cnout2' | type = 'CNOUT2')
  file = './cnlib/cn_out2.txt'
  endif
